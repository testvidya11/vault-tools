...
class HerokuIDTest < Vault::TestCase
  ...
  def test_to_uuid_with_app_id
    ...
    assert_equal(uuid,
                 Vault::Accounting::HerokuID.to_uuid('app#1234@heroku.com'))
  end
  ...
  def test_to_uuid_with_user_id
    ...
    uuid = UUIDTools::UUID.sha1_create(UUIDTools::UUID_URL_NAMESPACE, url).to_s
    assert_equal(uuid,
                 Vault::Accounting::HerokuID.to_uuid('user#1234@heroku.com'))
  end
  ...
  def test_uuid_with_invalid_heroku_id
    error = assert_raises(RuntimeError) do
      Vault::Accounting::HerokuID.to_uuid('invalid#1234@heroku.com')
    end
